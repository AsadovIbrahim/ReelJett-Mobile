...
const Onboarding = () => {
  ...
  const OnboardItem = ({ item }) => {
    return (
      <View className='items-center w-screen px-[40px] justify-center'>
        <Image
          source={item.image}
          resizeMode="contain"
  ...
  return (
    <ImageBackground className="flex-1 bg-[#252631]" >
      ...
      <FlatList onViewableItemsChanged={findIndex} showsHorizontalScrollIndicator={false} onScrollToIndexFailed={()=>{setFail(true)}} {...(!fail?{initialScrollIndex:3}:{})} pagingEnabled horizontal data={onboardingItems} 
        renderItem={({ item }) => (
